...
for v in range(0, 3001 + dv, dv):  # theoretical prediction
    theory.plot(v, (deltav / dv) * Natoms * 4 * pi *
                ((mass / (2 * pi * k * T))**1.5) * exp(-0.5 * mass * (v**2) /
                                                       (k * T)) * (v**2) * dv)
...
from checkCollisions import checkCollisions as cc

def checkCollisions():
    # fast
    r = Ratom
    return cc([ [p.x, p.y, p.z, r] for p in apos ])
...
while True:
    ...
    for ij in hitlist:
        ...
        d = (2 * Ratom) * cos(
            alpha) - dx  # distance traveled into the atom from first contact
